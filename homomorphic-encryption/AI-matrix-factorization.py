...
np.random.seed(42)
user_embeddings = np.random.rand(users, 2)  # 2 latent factors
item_embeddings = np.random.rand(items, 2)  # 2 latent factors
...
# Step 5: Compute encrypted recommendations (dot product)
encrypted_predictions = [enc_user.dot(item_embeddings.flatten().tolist()) for enc_user in encrypted_users]

# Step 6: Decrypt and display predictions
decrypted_predictions = [pred.decrypt() for pred in encrypted_predictions]
